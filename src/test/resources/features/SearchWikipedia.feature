...
Feature: Search Wikipedia
  ...
  @AllPokemon
  Scenario Outline: Search and compare pokemon data with poke API data
    Given User open google.com
    And User search "<keywordSearch>"
    And User select first search result
    Then User see pokemon data for "<pokemonName>" (pokemon number and name) are same with the poke API data
    Examples:
      | keywordSearch                 | pokemonName |
      | Pikachu Wikipedia Indonesia   | Pikachu     |
      | Charizard Wikipedia Indonesia | Charizard   |
      | Bulbasaur Wikipedia Indonesia | Bulbasaur   |
      | Mewtwo Wikipedia Indonesia    | Mewtwo      |
